...
@Component({selector: 'app-products-details', templateUrl: './products-details.component.html', styleUrls: ['./products-details.component.scss']})
export class ProductsDetailsComponent implements OnInit {
    ...
    constructor(private route : ActivatedRoute, private api : ApiService, private router : Router, private fb : FormBuilder, private sanitizer : DomSanitizer, private productAttributeService : ProductAttributeService, private productApplicationService : ProductApplicationService, private productService : ProductService) {
    ...
    async ngOnInit(): Promise < void > {
        ...
        await this.api.getId('product', this.id).subscribe(async (resp : any) => {
            ...
            await this.productService.getFiles(this.reg.code).subscribe((resp : any) => {
                if (resp.status) {
            ...
            await this.api.getCustom('typeProductAttribute', 'ListByTypeProductId', 'TypeProductId', this.reg.typeProductId).subscribe((resp : any) => {
                this.typeProductsAttributes = resp.data;

                this.api.getCustom('productattribute', 'listbyProductId', 'ProductId', this.id).subscribe((resp : any) => {
            ...
            await this.api.getCustom('productApplication', 'listbyProductId', 'ProductId', this.id).subscribe((resp : any) => {
                this.ProductAttribute.clear();
                if (resp.status) {
    ...
    Submit() {
        if (this.frm.invalid) {
            ...
            Swal.fire({title: 'Error', text: 'Hacen falta campos obligatorios', icon: 'error'});
            return;
        }
        Swal.fire({
                ...
                if (this.reg.id === '') {
                    ...
                    this.api.update('product', this.reg).subscribe((resp : any) => {
                        if (resp.error) {
                            ...
                            var controls = this.ProductAttribute.controls;
                            controls.forEach((element : any) => {
                                this.productAttributeService.getByProductIdAndTypeProductAttributeId(this.id, element.value.id).subscribe((resp : any) => {
                                    if (resp.status) {
                                        let productAttributeModel: ProductAttributeModel = {
                                            id: '',
                                            productId: this.reg.id,
                            ...
                            var app = this.ProductApplication.controls;
                            app.forEach((element : any) => {
                                this.productApplicationService.getByProductIdAndApplicationId(this.id, element.value.applicationId).subscribe((resp : any) => {
                                    if (resp.status) {
                                        let productApplication: ProductApplicationModel = {
                                            id: '',
                                            productId: this.reg.id,
    ...
    addApplication(applicationId : string = '', id : string = '', value : string = '', application : ApplicationModel) {
        const frmDetail = this.fb.group({applicationId: [applicationId], application: [application], id: [id], value: [value]});
        this.ProductApplication.push(frmDetail);
    }
    deleteApplication(idx : number) {
        var reg = this.ProductApplication.at(idx);
        if (reg.value.id != '') {
            this.api.delete('ProductApplication', reg.value.id).subscribe((resp : any) => {
                if (resp.status) {
    ...
    async selectedTypeProduct(id : string) {
        ...
        this.typeProductsAttributes = [];
        this.ProductAttribute.clear();
        await this.api.getCustom('typeProductAttribute', 'ListByTypeProductId', 'TypeProductId', id).subscribe((resp : any) => {
            this.typeProductsAttributes = resp.data;
            this.loadProductAttributes();
        });
    }
